...
def get_args_parser():
    ...
    parser.add_argument('--resnet_path', default='weights/resnet50.pth', type=str, metavar='MODEL',
                        help='Path of resnet model')
    parser.add_argument('--convnext_path', default='weights/open_clip_pytorch_model.bin', type=str, metavar='MODEL',
...
def main(args):
    ...
    if args.eval:
        ...
        for v_id, val in enumerate(vals):
            ...
            test_stats, acc, ap, auc = evaluate(data_loader_val, model, device, use_amp=args.use_amp)
            print(f"Accuracy of the model on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%, ap: {ap}, auc: {auc}.")
            print(f"test dataset is {val} acc: {acc}, ap: {ap},  auc: {auc}")
            print("***********************************")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            if args.model_ema and args.model_ema_eval:
                test_stats_ema, acc, ap, auc = evaluate(data_loader_val, model_ema.ema, device, use_amp=args.use_amp)
                print(f"Accuracy of the model EMA on {len(dataset_val)} test images: {test_stats_ema['acc1']:.1f}%, ap: {ap}, auc: {auc}")
