...
class AIDE_Model(nn.Module):
    def __init__(self, resnet_path, convnext_path, use_clip, use_resnet=True):
        ...
        self.fc = Mlp(2048 + (256 if self.use_clip else 0), 1024, 2)
    ...
    def forward(self, x):
        ...
        if self.use_resnet:
            ...
            x_vision = (x_min + x_max + x_min1 + x_max1) / 4
        else:
            x_vision = torch.zeros(b, 2048).to(x.device)  # 如果不使用 ResNet，返回零张量
        ...
        if self.use_clip:
            ...
            with torch.no_grad():
                ...
                feats = self.openclip_convnext_xxl(
                    tokens * (dinov2_std / clip_std) + (dinov2_mean - clip_mean) / clip_std
                )
                feats = self.avgpool(feats).view(tokens.size(0), -1)
                x_clip = self.convnext_proj(feats)

        if self.use_clip:
            x = torch.cat([x_clip, x_vision], dim=1)
        else:
            x = x_vision
