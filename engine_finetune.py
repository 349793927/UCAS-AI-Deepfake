...
import numpy as np
from sklearn.metrics import roc_auc_score  # 导入 AUC 计算函数
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    model.train(True)
    print("Train epoch started")  # ✅ 是否能看到这句？
    ...
    for data_iter_step, (samples, targets) in progress_bar:
        ...
        if use_amp:
            with torch.cuda.amp.autocast():
                output, output0, output1 = model(samples)
...
def evaluate(data_loader, model, device, use_amp=False):
    ...
    for index, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if use_amp:
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                output, output0, __ = model(images)
                if isinstance(output, dict):
                    output = output['logits']
                loss = criterion(output, target)
        else:
            output, output0, __ = model(images)  # [bs, num_cls]
            if isinstance(output, dict):
                output = output['logits']
    ...
    auc = roc_auc_score(y_true, y_pred)  # 计算 AUC
    
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, acc, ap, auc